...
using Unity.VisualScripting; //ToHexString
...
public class PlayerData : MonoBehaviour
{
    ...
    public void SaveArchiveData()
    {
        ...
        //오브젝트 파괴 개수 저장(색상별)
        int objectDestrovCountAll = 0;
        foreach(var item in dicDestroyObjectCount)
        {
            //아이템 키에 해당하는 색상 오브젝트 파괴 개수
            ...
            objectDestrovCountAll += item.Value;
        }
        //오브젝트 파괴 개수 저장 (통합)
        PlayerPrefs.SetInt(Constants.OBJECTDESTROYCOUNT, PlayerPrefs.GetInt(Constants.OBJECTDESTROYCOUNT) + objectDestrovCountAll);
    }
